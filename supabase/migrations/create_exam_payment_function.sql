-- Create function for deducting PromptCoins for exam attempts
-- This function ensures PromptCoin deduction happens atomically

CREATE OR REPLACE FUNCTION deduct_promptcoins_for_exam(
    p_user_id UUID,
    p_amount INTEGER,
    p_exam_level TEXT
) RETURNS VOID AS $$
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO promptcoin_transactions (
            ...
            'exam_attempt',
            -p_amount,
            CONCAT('Exam Attempt: ', p_exam_level),
            p_exam_level,
            'exam_attempt',
            NOW()
        );
