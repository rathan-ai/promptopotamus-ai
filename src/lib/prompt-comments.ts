...
/**
 * Prompt Comment System
 */
export class PromptCommentManager {
  private supabase;
  ...
  async getCommentStats(): Promise<{
    ...
    try {
      ...
      // Total comments
      const { count: totalComments } = await this.supabase
        .from('prompt_comments')
        .select('id', { count: 'exact' });

      // Comments today
      const { count: commentsToday } = await this.supabase
        .from('prompt_comments')
        .select('id', { count: 'exact' })
        .gte('created_at', today);

      // Comments this week
      const { count: commentsThisWeek } = await this.supabase
        .from('prompt_comments')
        .select('id', { count: 'exact' })
        .gte('created_at', weekAgo);

      // Top commented prompts (requires aggregation - simplified version)
      const { data: topPrompts } = await this.supabase
        .from('prompt_comments')
        .select(`
          prompt_id,
...
export const promptComments = new PromptCommentManager();